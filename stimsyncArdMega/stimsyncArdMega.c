...
#define IS_MEGA // we will use only usec mode
...
#define KEY_DOWN 129
#define KEY_UP 130
#define KEY_TRIGGER 131
#define OSC_HZ 132
#define OSC_CHANNELS 133
#define EEPROM_SAVE 134
#define NUM_ANALOG_KEYS 135
#define OSC_SUPER_SAMPLING 136
#define OSC_BITS_RESOLUTION 137
#define SOFTWARE_VERSION 138
#define MODE_KEY 169
#define MODE_USEC 181
#define MODE_OSC 162
...
const byte  kCmd1Set =  SET;
const byte kCmd1Get = GET;
const byte kCmd2Mode = MODE;
const byte kCmd2KeyDown = KEY_DOWN;
const byte kCmd2KeyUp = KEY_UP;
const byte kCmd2KeyTrigger = KEY_TRIGGER;
const byte kCmd2OscHz = OSC_HZ;
const byte kCmd2OscChannels = OSC_CHANNELS;
const byte kCmd2EEPROMSAVE = EEPROM_SAVE;
const byte kCmd2NumAnalogKeys = NUM_ANALOG_KEYS;
const byte kCmd2OscSuperSampling = OSC_SUPER_SAMPLING;
const byte kCmd2OscBitsResolution = OSC_BITS_RESOLUTION;
const byte kCmd2SoftwareVersion = SOFTWARE_VERSION;
const byte kCmd34ModeKey = MODE_KEY;
const byte kCmd34ModeuSec = MODE_USEC;
const byte kCmd34ModeOsc = MODE_OSC;
...
GET commands request the Arduino to report its current settings
 SET: 177 -Have Arduino Change Settings
  SET:MODE: 163 -change whether Arduino acts as a USB Keyboard, Microsecond Timer or Oscilloscope
   SET:MODE:KEYBOARD 169,169 - digital inputs mimic a USB keyboard [177,163,169,169] *** not implemented for Mega
   SET:MODE:USEC 181,181 - used for precise timing and to change keyboard mapping [177,163,181,181]
   SET:MODE:OSC 162,162- used to plot analog inputs [177,163,162,162] *** not implemented here
